...
internal val LocalHomePersian = staticCompositionLocalOf { true }
...
@Composable
fun t(english: String, persian: String): String =
    if (LocalHomePersian.current) localizePersian(persian) else english

fun tNoCompose(english: String, persian: String, persianMode: Boolean): String =
    if (persianMode) localizePersian(persian) else english

@Composable
fun homeLocalizedFont(): FontFamily? =
    if (LocalHomePersian.current) VazirmatnUiFd else null

@Composable
fun homeLocalizedTextStyle(base: TextStyle = LocalTextStyle.current): TextStyle {
